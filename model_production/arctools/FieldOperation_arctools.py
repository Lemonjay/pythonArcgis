...
fieldAdd = arcpy.GetParameterAsText(1)
building_id = arcpy.GetParameterAsText(2)
name = arcpy.GetParameterAsText(3)
city = arcpy.GetParameterAsText(4)
province = arcpy.GetParameterAsText(5)
typeSelect = arcpy.GetParameterAsText(6)
StatusSelect = arcpy.GetParameterAsText(7)
fieldReorderSelect = arcpy.GetParameterAsText(8)
arcpy.AddMessage(typeSelect)
...
class FieldOperation(object):
    ...
    def type_value_fill(self):
        ...
        type_field_dict = {'inwalls': '内墙', 'outwalls': '外墙', 'grounds': '地板', 'doors': '门', 'firedoors': '防火门',
                           'elevators': '客用电梯', 'firelifts': '消防电梯', 'stairs': '疏散楼梯', 'wireframe': '线框',
                           'inhydrants': '室内消火栓', 'fire_curtains': '防火卷帘', 'handalarmsys': '手动报警器'}
...
for dataSet in dataSets:
    ...
    for ft in arcpy.ListFeatureClasses():
        arcpy.AddMessage(ft + ' Start')
        fldOp = FieldOperation(ft)

        # Add and delete field
        if fieldAdd:
            arcpy.AddMessage('Field Add')
            fldOp.feature_field_op()
            arcpy.AddMessage('         ')

        # Field fill
        arcpy.AddMessage('Field fill')
        fldOp.constant_value_fill(building_id, name, city, province)
        arcpy.AddMessage('         ')

        # Type Field fill
        if typeSelect:
            arcpy.AddMessage('Type Field fill')
            fldOp.type_value_fill()
            arcpy.AddMessage('         ')

        # Status Field fill
        if StatusSelect:
            arcpy.AddMessage('Status Field fill')
            fldOp.status_value_fill()
            arcpy.AddMessage('         ')

        # Field Reorder
        if fieldReorderSelect:
            arcpy.AddMessage('Field Reorder')
            fldOp.field_reorder()
            arcpy.AddMessage('         ')

        arcpy.AddMessage(ft + ' End')
        arcpy.AddMessage('         ')
